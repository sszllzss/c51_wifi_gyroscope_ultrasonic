...
int8_t UARTx_CheckPosRxBuff(enum USARTx com,uint8_t pos,uint8_t *buff,uint16_t len)//并不从队列里取出数据只是查看一下数据
{
    uint16_t i=0;
...
int8_t UARTx_CheckRxBuff(enum USARTx com,uint8_t *buff,uint16_t len)//并不从队列里取出数据只是查看一下数据
{
    uint16_t i=0;
